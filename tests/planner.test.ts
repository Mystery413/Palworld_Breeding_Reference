...
test("仅有同种配方的目标会被直接识别，存在任意跨种配方则不会误判", () => {
  const selfOnly = fixture([["T", "T", "T", "WILDCARD", "WILDCARD"]]);
  assert.deepEqual(selfBreedingOnlyCombo(selfOnly, "T"), ["T", "T", "T", "WILDCARD", "WILDCARD"]);
  const mixed = fixture([
    ["T", "T", "T", "WILDCARD", "WILDCARD"],
    ["T", "A", "B", "WILDCARD", "WILDCARD"],
  ]);
  assert.equal(selfBreedingOnlyCombo(mixed, "T"), null);
  assert.equal(selfBreedingOnlyCombo(mixed, "missing"), null);
});
