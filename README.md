...
- 公共配种资料由 GitHub Pages 静态缓存提供，Supabase 只保存共享用户库存
- 指定目标使用反向约束搜索，并设有浏览器安全上限和可停止的后台计算
...
构建时会自动把完整数据拆成首屏核心数据和按帕鲁读取的栖息点文件：
...
路线规划直接查 1.0 组合表，不用旧版配种力公式猜测结果。指定目标时只扩展仍可在剩余代数内通向目标的物种；为防止高代数产生数亿候选，单次搜索最多保留 100,000 个状态、返回综合排序最优的 240 条路线，并在触发上限时明确提示。默认限制页面实际展示的配种步骤为四步，用户可以手动提高上限；允许补抓时，普通野生与 Alpha Boss 分别核验，常见野生等级高于“玩家当前等级＋8”的来源会被严格排除。常见等级取主要栖息点最集中的等级簇，排除世界树高等级点等离群值。可行来源会按等级计入路线成本，Alpha 额外增加难度惩罚。普通蛋糕的干净词条池概率按 1/2/3/4 个目标词条分别为 40%/24%/12%/10% 估算；功能蛋糕、突变和未公布倍率不会被伪装成精确概率。
